...
def vab_char_mapping(vocab_file):
    char_to_id = {}
    with open(vocab_file, "r", encoding='utf-8') as f:
        i = 0
        for line in fr:
            if line.strip() not in char_to_id:
                char_to_id[line.strip()] = i
                i += 1
            else:
                continue
    char_to_id["<PAD>"] = i
...
def vab_tag_mapping(tags):
    n = len(tags)
    id_to_tag = {i: v for i, v in enumerate(tags)}
    id_to_tag[n] = '[SEP]'
    id_to_tag[n + 1] = '[CLS]'
    id_to_tag[n + 2] = 'X'
...
def input_from_line(line, max_seq_length, tag_to_id):
    ...
    #print(string, segment_ids, ids, mask)
    return [string, segment_ids, ids, mask, label_ids]
    
def data_generator(path, batch_size, max_seq_length, tag_to_id):
    while 1:
        f = open(path)
        cnt = 0
        input_ids = []
        ...
        Targets = []
        for line in f:
            all_info = input_from_line(line, max_seq_length, tag_to_id)
            input_id = all_info[2]
            input_mask = all_info[3]
            segment_id = all_info[1]
            Target = all_info[3]
            ...
            if cnt == batch_size:
                cnt = 0
                yield ([np.array(Targets), np.array(input_ids), np.array(input_masks), np.array(segment_ids)], np.array(lines))
                input_ids = []
                input_masks = []
                segment_ids = []
                Targets = []
